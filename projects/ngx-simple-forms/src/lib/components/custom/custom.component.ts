...
export class CustomComponent implements OnInit {
  ...
  @Input('fields') fields: { [key: string]: Field } = {};
        ...
        new FormControl(
          typeof field.value !== 'undefined'
            ...
            : typeof this.model === 'object' &&
              this.model !== null &&
              typeof this.model[key] !== 'undefined'
            ? this.model[key]
            : field.defaultValue,
          field.validators instanceof Array ? field.validators : []
        )
      );
    }
  ...
  getFieldByKey(key: string): Field | null {
    if (
      !(
        ...
        (typeof this.fields === 'object' && this.fields !== null)
      )
    ) {
      return null;
    }
  ...
  hasSubmitHandleButton() {
    if (
      !(
        this.buttons instanceof Array ||
        (typeof this.buttons === 'object' && this.buttons !== null)
      )
    ) {
      return false;
    }
    ...
    for (const key in this.buttons) {
      if (
        typeof (this.buttons[key] as ButtonPresubmit).handle === 'function' &&
        (this.buttons[key] as ButtonPresubmit).type === 'submit'
      ) {
        return true;
      }
    }
  ...
  async onSubmit() {
    ...
    try {
      switch (
        typeof this.submit.method === 'string' && this.submit.method.length
          ? this.submit.method.toUpperCase()
          : 'GET'
      ) {
        case 'GET': {
          this.successResult = await this.http
            .get(this.submit.url, { params: this.form.value })
            .toPromise();
          break;
        }
        case 'PATCH': {
          if (this.hasFileField()) {
            this.successResult = await this.http
              .patch(this.submit.url, formData, { headers: headers })
              .toPromise();
          } else {
            this.successResult = await this.http
              .patch(this.submit.url, this.form.value)
              .toPromise();
          }
          break;
        }
        case 'PUT': {
          if (this.hasFileField()) {
            this.successResult = await this.http
              .put(this.submit.url, formData, { headers: headers })
              .toPromise();
          } else {
            this.successResult = await this.http
              .put(this.submit.url, this.form.value)
              .toPromise();
          }
          break;
        }
        case 'DELETE': {
          this.successResult = await this.http
            .delete(this.submit.url, { params: this.form.value })
            .toPromise();
          break;
        }
        case 'POST':
        default: {
          if (this.hasFileField()) {
            this.successResult = await this.http
              .post(this.submit.url, formData, { headers: headers })
              .toPromise();
          } else {
            this.successResult = await this.http
              .post(this.submit.url, this.form.value)
              .toPromise();
          }
          break;
        }
      ...
      if (
        typeof this.submit === 'object' &&
        this.submit !== null &&
        typeof this.submit.success === 'object' &&
      ...
      if (
        typeof this.submit === 'object' &&
        this.submit !== null &&
        typeof this.submit.error === 'object' &&
